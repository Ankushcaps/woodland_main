...
const ProductList = () => {
  ...
  async function fetchProductDetails() {
    ...
    try {
      const response = await axios.get(
        `https://verwgrfk5k.execute-api.ap-south-1.amazonaws.com/prod/category/${slug}`
      );
  ...
  //     setProductlisting(sortedProducts);
  //     pushProductListingEvents(sortedProducts);
  //     setPlpdetails(response.data.category);
  //     setLoading(false);
  //   } catch (error) {
  //     console.error(error);
  ...
  return (
    <Box>
      ...
      <Head>
        ...
        <meta property="og:title" content={plpdetails?.seo_title} />
        <meta property="og:description" content={plpdetails?.seo_description} />
        <meta property="og:image" content={plpdetails?.image} />
      </Head>
      {loading ? (
        <Box>
          <Loading />
        ...
        >
          ...
          <p className={style.breadcrum}>
            HOME / SHOP / {plpdetails?.gender} / {productlisting[0]?.category}
          </p>
          <Box
            sx={{
              display: {
